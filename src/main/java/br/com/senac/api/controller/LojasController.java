...
public class LojasController {
    ...
    public ResponseEntity<List<LojasResponse>> carregarLojas() {
        ...
        List<LojasResponse> out = retorno.stream().map(LojasMapper::lojasToLojasResponse).toList();
        return  ResponseEntity.ok().body(out);
    }

    @PostMapping("/")
    @CrossOrigin
    public ResponseEntity<LojasResponse> criarLoja(@RequestBody LojasRequest loja) {
        Lojas envio = LojasMapper.lojasRequestToLojas(loja);
        Lojas retorno = lojasRepository.save(envio);
    ...
    @GetMapping("/{id}")
    @CrossOrigin
    public ResponseEntity<LojasResponse> carregarLojasById(@PathVariable Long id) {
        Lojas retorno = lojasRepository.findById(id).get();
    ...
    @CrossOrigin
    public ResponseEntity<List<LojasResponse>> carregarLojaByNome(@PathVariable String nome) {
        List<Lojas> retorno = lojasRepository.findByNome(nome);
